...
/**
 * Video-to-Video (A2E)
 * Transform existing video with new style/characters
 * Uses A2E's video editing capabilities
 */
export async function POST(req: NextRequest) {
  try {
    ...
    // Call A2E Video-to-Video API
    // Using userVideo2Video endpoint for style transfer/editing
    const response = await fetch('https://video.a2e.ai/api/v1/userVideo2Video/start', {
      method: 'POST',
      headers: {
    ...
    console.log('A2E V2V Response status:', response.status);
    const responseText = await response.text();
    console.log('A2E V2V Response:', responseText.substring(0, 500));
    ...
    console.log('A2E V2V Task ID:', taskId);
    ...
    while (attempts < maxAttempts && !resultUrl) {
      ...
      const statusResponse = await fetch(`https://video.a2e.ai/api/v1/userVideo2Video/${taskId}`, {
        headers: {
          'Authorization': `Bearer ${process.env.A2E_API_KEY}`,
        },
      });
      ...
      console.log(`V2V polling attempt ${attempts + 1}:`, statusData.data?.current_status);
      ...
      if (statusData.data?.current_status === 'failed') {
        return NextResponse.json(
          { error: 'Video-to-Video generation failed: ' + (statusData.data?.failed_message || 'Unknown error') },
          { status: 500 }
        );
      }
    ...
    // Deduct credits (120 credits for v2v - more expensive than i2v)
      ...
      if (profile && profile.credits >= creditCost) {
        ...
        await supabase
          ...
          .insert({
            ...
            description: 'Video to Video (A2E)',
          });
      } else {
        return NextResponse.json(
    ...
    console.error('A2E V2V error:', error);
    return NextResponse.json(
