...
/**
 * AtlasCloud Wan 2.6 Image-to-Video
 * Latest model with 1080p support, up to 15s duration, audio support
...
export async function POST(req: NextRequest) {
  try {
    ...
    const creditCost = resolution === '1080p' ? 60 : 45; // Higher cost for 1080p
    ...
    if (userId && !isSuperAdmin) {
      ...
      if (!profile || profile.credits < creditCost) {
        return NextResponse.json(
          { error: `Insufficient credits. Need ${creditCost} credits for I2V.` },
          { status: 402 }
        );
      }
      ...
      await supabase
        ...
        .insert({
          ...
          description: 'AtlasCloud Wan 2.6 I2V',
        });
    }
    ...
    console.log('AtlasCloud Wan 2.6 I2V Request:', {
      hasImage: !!image,
    ...
    const response = await fetch(ATLASCLOUD_API_URL, {
      ...
      body: JSON.stringify({
        ...
        image,
        prompt: prompt || 'cinematic motion, smooth movement, high quality',
        negative_prompt: negativePrompt || 'blur, static, low quality',
        duration,
        resolution,
        seed,
        shot_type: shotType,
    ...
    console.log('AtlasCloud Wan 2.6 I2V initial response:', result);
    ...
    console.log('AtlasCloud Wan 2.6 I2V output:', videoUrl);
    ...
    console.error('AtlasCloud Wan 2.6 I2V error:', error);
    return NextResponse.json(
