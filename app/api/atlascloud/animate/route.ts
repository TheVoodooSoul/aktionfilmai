  ...
  try {
    ...
    if (userId && !isSuperAdmin) {
      ...
      if (!profile || profile.credits < creditCost) {
        return NextResponse.json(
          { error: `Insufficient credits. Need ${creditCost} credits for animate.` },
          { status: 402 }
        );
      }
      ...
      await supabase
        ...
        .insert({
          ...
          description: 'AtlasCloud Animate',
        });
    }

    console.log('AtlasCloud Animate Request:', {
      hasImage: !!image,
    ...
    const response = await fetch(ATLASCLOUD_API_URL, {
      ...
      body: JSON.stringify({
        ...
        image,
        prompt: prompt || 'subtle natural motion, breathing, slight movement',
        resolution,
        seed,
        mode,
      }),
    ...
    console.log('AtlasCloud Animate initial response:', result);
    ...
    if (!response.ok || !result.data?.id) {
      ...
      return NextResponse.json(
        { error: result.message || 'Failed to start animation' },
        { status: 500 }
      );
    }
    ...
    while (attempts < maxAttempts && !videoUrl) {
      ...
      if (pollResult.data?.status === 'completed' || pollResult.data?.status === 'succeeded') {
        ...
        return NextResponse.json(
          { error: pollResult.data.error || 'Animation failed' },
          { status: 500 }
        );
      }
    ...
    if (!videoUrl) {
      ...
      return NextResponse.json(
        { error: 'Animation timed out. Credits refunded.' },
        { status: 408 }
      );
    }

    console.log('AtlasCloud Animate output:', videoUrl);
    ...
    console.error('AtlasCloud Animate error:', error);
    return NextResponse.json(
