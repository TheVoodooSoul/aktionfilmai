...
export async function POST(req: NextRequest) {
  try {
    ...
    // Build enhanced prompt with character info (names + outfits)
    let enhancedPrompt = prompt || 'transform this image into a high quality action scene';

    // If character info provided, add them to the prompt
    ...
    console.log('Dzine Request:', {
      hasToken: !!process.env.DZINE_API_TOKEN,
      hasPrompt: !!prompt,
      promptValue: enhancedPrompt,
      hasImage: !!image,
      characterCount: characterInfo?.length || 0,
    });
    ...
    while (attempts < maxAttempts && !imageUrl) {
      ...
      if ((statusData.data?.status === 'succeeded' || statusData.data?.status === 'succeed') && statusData.data?.generate_result_slots?.length > 0) {
